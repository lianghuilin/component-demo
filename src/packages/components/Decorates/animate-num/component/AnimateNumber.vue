<template>
  <span v-if="num != undefined" v-text="num" :style="numColor"></span>
</template>
<script>
...
export default {
  ...
  methods: {
    updateNumber(state) {
      const resetState = state.x ? state : state.state;
      this.num = this.formatter ? this.formatter(resetState.x) : resetState.x;
      this.numColor = {
        color: `rgb(${parseInt(state.r)}, ${parseInt(state.g)}, ${parseInt(state.b)})`,
      };
    },
    makeOptions() {
      let from =
        typeof this._from === "string"
          ...
          : { x: this._from };
      let to =
        typeof this._to === "string" ? { x: parseFloat(this._to) } : { x: this._to };
      if (this._fromColor && this._toColor) {
    ...
    reset(from, to, fromColor, toColor) {
      this.options.from =
        typeof from === "string" ? { x: parseFloat(from) } : { x: from };
